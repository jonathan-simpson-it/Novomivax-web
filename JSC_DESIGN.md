...
- Sage-green accent (`#80988f`) is the only colour, used sparingly for eyebrows, hover states, and visual rhythm
...
:root {
  ...
  --color-muted: #5c5e56; /* secondary text: olive-grey */

  /* Structure */
  --color-line: #d6d8d1; /* borders, dividers, hr */

  /* Accent */
  --color-accent: #80988f; /* sage green: the only colour */
...
- Section eyebrows (`.section-eyebrow`) are always `0.72rem`, uppercase, `letter-spacing: 0.1em`, accent colour
- Buttons are `0.78rem`, uppercase, `letter-spacing: 0.06em`
- Maximum line length for body text should be ~40–50rem (keep readable)
- Body text colour is always `var(--color-muted)` for paragraphs/lists, `var(--color-ink)` for headings
...
- Eyebrow (`.section-eyebrow`): `0.72rem`, uppercase, accent colour
...
<footer class="site-footer">
  <div class="container footer-inner">
    <div>    <!-- Brand  2fr -->
      <p class="footer-brand-large">Brand name</p>
...
**`.footer-heading`**: `0.75rem`, uppercase, letter-spacing `0.08em`, colour `var(--color-accent)`. Used as the title for each column.

**`.footer-brand-large`**: serif (`--font-display`), `clamp(1.6rem, 4.5vw, 3.5rem)`, near-black (`--color-ink`).
...
- Every page must use `BaseLayout` which provides OG tags, Twitter cards, canonical URL, favicon, and organisation JSON-LD
...
Typography and spacing do the heavy lifting. The hierarchy of a page should be legible even without colour: it's in the scale, weight, and position of type. Our designs read like editorial layouts because information design comes first.
...
Transitions are fast (220ms), subtle, and always tied to interaction. No decorative animations, no scroll-triggered reveals for the sake of it. Hover effects lift cards 2px. Button hover changes background colour. That's it.
...
- **Description**: 1-3 sentences that expand the title, written in muted colour
...
- Colour contrast: ink-on-surface passes AAA, muted-on-surface passes AA
...
### 15. File Organisation & Conventions
...
## Part III: Enosis Product Addendum

> Product-specific patterns for the ENOSIS demo (Next.js + Tailwind). The base system above governs tokens, type, spacing, and components. This addendum defines the surfaces the agency doc does not cover. See `PRODUCT.md` for product strategy.

### 18. Product-Specific Surfaces

#### 18.1 The Dark Hero Stage

- The landing hero and the processing theater render on a **dark ink stage** (`#161714`), not cream. This is the one sanctioned deviation from the all-cream base, matching the base spec's "full-viewport video with dark gradient overlay".
- On dark: text is `#f4f4ef`, muted text is sage-soft (`#e3e9e6` at ~70% opacity), accent stays `#80988f`. Borders on dark are `rgba(244, 244, 239, 0.14)`.
- The stage shows the transformation: paper artifacts (scan lines, ruled-paper texture, ink) resolving into clean structured JSON on the right. The JSON block is mono, sage-highlighted keys, ink-on-cream card.

#### 18.2 Processing Theater (demo engine sequence)

- The 5-engine sequence (DocFormer-Trade, UncertaintyGuard, HierarchicalHS, MetaSchema, TradeBench) runs as a vertical list of engine rows.
- States: waiting (40% opacity), active (accent border + subtle glow), done (emerald check).
- Timing: 700ms per engine on first pass. This is theater with an explanation purpose, sanctioned slower than the 220ms UI rule.
- Exit must be faster than enter: result cards enter at 500ms, next-step transitions at 250ms.

#### 18.3 Confidence Bars

- Bar: 10px tall, rounded, `--color-line` track.
- Fill colors: emerald `>=0.95` (auto-approved), amber `0.80-0.94` (needs review), red `<0.80` (review required).
- **Zero is never a grey bar.** A missing field shows the label "Not detected" with muted text, not a 0% bar that reads as broken.
- Threshold chip next to the bar: "Guarantee: 95% (p<0.05)" on the Verification step.

#### 18.4 JSON Export Viewer

- Export payloads render as a read-only code block: `--font-mono` 0.78rem, ink-on-surface, 1px `--color-line` border, radius `--radius-lg`.
- Keys in accent, strings in ink, numbers in muted. No syntax-highlight library; hand-rolled spans keep the aesthetic.

#### 18.5 Document Cards (demo intake)

- 6 sample docs as selectable cards: 1px border, radius `--radius-lg`, surface bg, mono type badge top-right (TXT/CSV/OCR/PDF/XLSX).
- Selected state: accent border + `--accent-soft/50` fill. "Selected" caption in accent below the preview.
- Preview strip: 3 lines of mono text in a `--accent-soft/20` well, truncated.

### 19. Motion Tokens (override 220ms default where noted)

```css
--ease-out-quart: cubic-bezier(0.16, 1, 0.3, 1); /* standard entrance */
--ease-in-out: cubic-bezier(0.77, 0, 0.175, 1); /* on-screen movement */
```

- UI micro-interactions: 180-220ms, ease-out-quart.
- Button press: `scale(0.97)` at 160ms.
- Stagger between list items: 40ms.
- All motion on transform/opacity only. `prefers-reduced-motion` collapses to instant states.

### 20. Copy Constraints (stop-slop)

- No em dashes. No "not X, it's Y" contrasts. No throat-clearing.
- Every claim carries a number (95%, p<0.05, 120s, HK$349) or it does not ship.
- The mom story is told once, on the landing hero, in her voice. Never repeated on product screens.
- Never publish benchmark numbers that are not reproducible live. (Benchmark table stays out of the product.)
